//! Behaviour matrix for MD093. Every reported row asserts what was found and
...
use super::MD093NoFormattingInHeadings;
use super::md093_config::MD093Config;
use crate::config::MarkdownFlavor;
use crate::lint_context::LintContext;
use crate::rule::{LintWarning, Rule};

fn rule() -> MD093NoFormattingInHeadings {
    MD093NoFormattingInHeadings::default()
}

fn with_config(code: bool, strong: bool, emphasis: bool) -> MD093NoFormattingInHeadings {
    MD093NoFormattingInHeadings::from_config_struct(MD093Config { code, strong, emphasis })
}

fn check_with(rule: &MD093NoFormattingInHeadings, content: &str) -> Vec<LintWarning> {
...
fn assert_messages(content: &str, expected: &[&str]) {
    ...
    assert_eq!(
        rule().fix(&ctx).unwrap(),
        content,
        "MD093 reports only, so fix must never rewrite {content:?}"
    );
}
