//! Configuration for MD093 (no formatting in headings).
...
/// Configuration for MD093. One switch per construct, because the three have
...
impl RuleConfig for MD093Config {
    const RULE_NAME: &'static str = "MD093";
}
...
mod tests {
    ...
    fn every_construct_is_reported_by_default() {
        let config = MD093Config::default();
        assert!(config.code);
    ...
    fn a_single_construct_can_be_turned_off() {
        let config: MD093Config = toml::from_str("code = false\n").unwrap();
        assert!(!config.code);
        assert!(config.strong);
        assert!(config.emphasis);
    }
