...
///
/// In width-driven reflow modes, an unwrappable standalone link keeps its
/// paragraph boundary. Sentence-driven modes keep it with the sentence above.
///
pub(crate) fn standalone_link_ends_paragraph(ctx: &LintContext, line_num: usize, config: &MD013Config) -> bool {
    !config.strict
        ...
        && is_unwrappable_standalone_link_or_image(ctx, line_num, config)
}
