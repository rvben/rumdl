...
const MAX_INPUT_SIZE: usize = 1024 * 1024;
    ...
    static EMPHASIS_PATTERN: LazyLock<Regex> = LazyLock::new(|| Regex::new(
        r"\*+([^*]*?)\*+|^__([^_]+?)__|^_([^_]+?)_$|(?:\s)__([^_]+?)__(?:\s)|(?:\s)_([^_]+?)_(?:\s)"
    ).unwrap());
    static CODE_PATTERN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"`+([^`]*?)`+").unwrap());
    static LINK_PATTERN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\[([^\]]*?)\]\([^)]*\)|\[([^\]]*?)\]\[[^\]]*?\]").unwrap());

    // Control character and dangerous Unicode filtering
    static CONTROL_CHARS: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[\x00-\x1F\x7F-\x9F\u200B-\u200D\uFEFF]").unwrap());

    // Whitespace normalization (tabs, Unicode spaces)
    static WHITESPACE_NORMALIZE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[\t\u00A0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000]").unwrap());
