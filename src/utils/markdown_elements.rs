            ...
            if let Some(captures) = CODE_BLOCK_START.captures(line) {
                if !in_code_block {
                    ...
                    blocks.push(MarkdownElement {
                        ...
                        end_line: i,
                        text: content.lines().skip(block_start).take(i - block_start + 1).collect::<Vec<&str>>().join("\n"),
...
mod tests {
    ...
    fn test_heading_to_fragment() {
        ...
        assert_eq!(MarkdownElements::heading_to_fragment("Complex: (Header) 123"), "complex-header-123");
    }
