...
impl<'a> LintContext<'a> {
    ...
    /// Get inline config overrides for a specific rule from configure-file comments
    ///
    /// Returns the JSON value from inline comments like:
    /// `<!-- rumdl-configure-file { "MD013": { "line_length": 120 } } -->`
    ///
    /// Returns `None` if no inline config is present for the rule.
    pub fn get_rule_inline_config(&self, rule_name: &str) -> Option<&serde_json::Value> {
        self.inline_config.get_rule_config(rule_name)
    }

    /// Get effective configuration for a rule by merging inline config overrides
    ///
    /// This is the recommended way for rules to support inline configuration.
    /// It takes the rule's base configuration and merges any inline overrides from
    /// configure-file comments.
    ///
    /// # Example
    ///
    /// ```ignore
    /// // In a rule's check() method:
    /// let effective_config = ctx.get_effective_config("MD064", &self.config);
    /// if effective_config.allow_sentence_double_space {
    ///     // Use the (possibly overridden) config value
    /// }
    /// ```
    ///
    /// # Type Parameters
    ///
    /// * `C` - The configuration type, must implement Serialize, DeserializeOwned, and Clone
    pub fn get_effective_config<C>(&self, rule_name: &str, base_config: &C) -> C
    where
        C: serde::Serialize + serde::de::DeserializeOwned + Clone,
    {
        if let Some(json_override) = self.get_rule_inline_config(rule_name) {
            // Serialize base config to JSON Value
            if let Ok(mut base_json) = serde_json::to_value(base_config) {
                // Merge override into base (override takes precedence)
                if let Some(base_obj) = base_json.as_object_mut() {
                    if let Some(override_obj) = json_override.as_object() {
                        for (key, value) in override_obj {
                            // Normalize the key to match what serde serializes
                            // Config structs typically use #[serde(rename_all = "kebab-case")]
                            // so both "allow_sentence_double_space" and "allow-sentence-double-space"
                            // should map to the same field

                            // Convert snake_case to kebab-case to match serialized format
                            let normalized_key = key.replace('_', "-");

                            // Remove any existing variant of the key to avoid duplicates
                            let snake_key = normalized_key.replace('-', "_");
                            base_obj.remove(&snake_key);
                            base_obj.remove(&normalized_key);

                            // Insert with kebab-case (standard serialization format)
                            base_obj.insert(normalized_key, value.clone());
                        }
                    }
                    // Deserialize back to config type
                    if let Ok(merged) = serde_json::from_value(base_json) {
                        return merged;
                    }
                }
            }
