...
fn test_detect_front_matter_type() {
    ...
    assert_eq!(FrontMatterUtils::detect_front_matter_type(malformed_content1), FrontMatterType::Malformed);
    
    let malformed_content2 = "-- -\ntitle: Test Document\ndate: 2023-04-01\n-- -\n# Heading";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(malformed_content2), FrontMatterType::Malformed);
    
    // No front matter
    let no_front_matter = "# Heading\nThis is content.";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(no_front_matter), FrontMatterType::None);
    
    // Empty document
    let empty_content = "";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(empty_content), FrontMatterType::None);
    
    // Too short for front matter
    let short_content = "---\ntitle";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(short_content), FrontMatterType::None);
}
...
fn test_get_front_matter_field_value() {
    // YAML front matter
    let yaml_content = "---\ntitle: Test Document\ndate: 2023-04-01\n---\n# Heading";
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(yaml_content, "title"), Some("Test Document"));
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(yaml_content, "date"), Some("2023-04-01"));
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(yaml_content, "author"), None);
    
    // TOML front matter
    let toml_content = "+++\ntitle = \"Test Document\"\ndate = 2023-04-01\n+++\n# Heading";
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(toml_content, "title"), Some("Test Document"));
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(toml_content, "date"), Some("2023-04-01"));
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(toml_content, "author"), None);
    
    // No front matter
    let no_front_matter = "# Heading\nThis is content.";
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(no_front_matter, "title"), None);
    
    // Edge cases
    let empty_content = "";
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(empty_content, "title"), None);
    
    let short_content = "---\ntitle";
    assert_eq!(FrontMatterUtils::get_front_matter_field_value(short_content, "title"), None);
}
...
fn test_complex_front_matter_scenarios() {
    ...
    assert_eq!(FrontMatterUtils::detect_front_matter_type(complex_yaml), FrontMatterType::Yaml);
...
# Heading";
    ...
    assert_eq!(FrontMatterUtils::detect_front_matter_type(toml_with_types), FrontMatterType::Toml);
...
# Heading";
    ...
    assert_eq!(quoted_fields.get("description"), Some(&"'Single quoted description'".to_string()));
...
# Not really front matter";
    ...
    assert_eq!(FrontMatterUtils::detect_front_matter_type(mixed_delimiters), FrontMatterType::None);
}
...
fn test_front_matter_distinction() {
    ...
    // Standard YAML
    let standard_yaml = "---\ntitle: YAML Document\n---\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(standard_yaml), FrontMatterType::Yaml);
    
    // Standard TOML
    let standard_toml = "+++\ntitle = \"TOML Document\"\n+++\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(standard_toml), FrontMatterType::Toml);
    
    // YAML with +++ content (should still be YAML)
    let yaml_with_plus = "---\ntitle: Document with +++\n---\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(yaml_with_plus), FrontMatterType::Yaml);
    
    // TOML with --- content (should still be TOML)
    let toml_with_dash = "+++\ntitle = \"Document with ---\"\n+++\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(toml_with_dash), FrontMatterType::Toml);
    
    // Ambiguous syntax (first delimiter wins)
    let yaml_first = "---\ntitle: YAML\n+++\nMore content\n---\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(yaml_first), FrontMatterType::Yaml);
    
    let toml_first = "+++\ntitle = \"TOML\"\n---\nMore content\n+++\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(toml_first), FrontMatterType::Toml);
    
    // Whitespace before front matter (should be no front matter)
    let space_before_yaml = " ---\ntitle: YAML\n---\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(space_before_yaml), FrontMatterType::None);
    
    let space_before_toml = " +++\ntitle = \"TOML\"\n+++\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(space_before_toml), FrontMatterType::None);
    
    // Missing closing delimiter
    let unclosed_yaml = "---\ntitle: Unclosed YAML\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(unclosed_yaml), FrontMatterType::None);
    
    let unclosed_toml = "+++\ntitle = \"Unclosed TOML\"\n# Content";
    assert_eq!(FrontMatterUtils::detect_front_matter_type(unclosed_toml), FrontMatterType::None);
} 