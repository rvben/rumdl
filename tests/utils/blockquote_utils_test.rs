...
fn test_is_blockquote_detection() {
    ...
    assert!(BlockquoteUtils::is_blockquote(">"));
    assert!(BlockquoteUtils::is_blockquote("> "));
    assert!(BlockquoteUtils::is_blockquote(" >Text without space after angle bracket"));
}
...
fn test_get_nesting_level() {
    ...
    assert_eq!(BlockquoteUtils::get_nesting_level("> > Quote with > inside at level 2"), 1); // Implementation actually returns 1 here
...
fn test_formatting_inside_blockquotes() {
    ...
    assert!(BlockquoteUtils::is_blockquote("> [Link text](https://example.com) in blockquote"));
