...
fn create_test_config() -> Config {
    ...
    md004_values.insert("style".to_string(), toml::Value::String("asterisk".to_string()));
...
fn apply_rule_configs(rules: &mut Vec<Box<dyn Rule>>, config: &Config) {
    for rule in rules.iter_mut() {
        ...
        if rule_name == "MD013" {
            ...
            *rule = Box::new(MD013LineLength::new(line_length, code_blocks, tables, headings, strict));
        }
...
"#;
    ...
    let md004_warnings: Vec<_> = warnings.iter()
        .filter(|w| w.rule_name == Some("MD004"))
        .collect();
    assert_eq!(md004_warnings.len(), 2, "MD004 should trigger twice (dash and plus markers)");
    
    // Make sure the non-configured rule (MD001) still works normally
    let md001_warnings = warnings.iter()
        .filter(|w| w.rule_name == Some("MD001"))
        .count();
    assert_eq!(md001_warnings, 0, "MD001 should not trigger on this content");
}
...
fn test_config_priority() {
    ...
    let md013_warnings = warnings.iter()
        .filter(|w| w.rule_name == Some("MD013"))
        .count();
    assert_eq!(md013_warnings, 0, "MD013 should not trigger with configured line_length 120");
...
fn test_partial_rule_config() {
    ...
    let mut rules: Vec<Box<dyn Rule>> = vec![
        Box::new(MD013LineLength::new(80, true, false, true, false)),
    ];
...
"#;
    ...
    assert_eq!(md004_warnings, 1, "MD004 should flag asterisk when style is set to dash");
...
"#;
    ...
    assert_eq!(rules[0].name(), "MD013", "The only enabled rule should be MD013");
