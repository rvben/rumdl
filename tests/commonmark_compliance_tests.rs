...
fn verify_commonmark_elements(content: &str, filename: &str) {
    match filename {
        ...
        "code-blocks.md" => {
            ...
            assert!(content.contains("```rust"), "Missing code block with language");
...
fn verify_document_structure(content: &str) {
    ...
    assert!(content.contains("title") && content.contains("author"), 
            "Front matter content lost");
    
    // Verify paragraph content
    assert!(content.contains("*emphasized*") && content.contains("**strong**"), 
    ...
    assert!(content.contains("---") || content.contains("***") || content.contains("___"), 
