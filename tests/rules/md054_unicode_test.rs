...
"#;
    ...
    assert!(result.len() > 0, "Restricted styles with Unicode should generate warnings");
...
"#;
    ...
    let result = rule_restricted.check(content_mixed).unwrap();
    assert!(result.len() > 0, "Restricted styles with Unicode images should generate warnings");
} 