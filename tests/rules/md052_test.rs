...
fn test_inline_code_spans() {
    ...
    assert_eq!(result.len(), 0, "Reference link in inline code span should be ignored");
    
    // Test with multiple code spans and mix of valid and invalid references
    let content = "`[invalid][missing]` and [valid][id] and `[another][nowhere]`\n\n[id]: http://example.com";
    
    let result = rule.check(content).unwrap();
    
    // Only valid references should be checked, the ones in code spans should be ignored
    assert_eq!(result.len(), 0, "Only references outside code spans should be checked");
    ...
    assert_eq!(result.len(), 1, "Only real invalid references should be caught");
    assert_eq!(result[0].line, 1, "Warning should be on line 1");
    assert!(result[0].message.contains("missing"), "Warning should be about 'missing'");
    ...
    assert_eq!(result.len(), 0, "Shortcut references in code spans should be ignored");
}
