...
fn test_with_front_matter() {
    ...
    let result = rule.check(content).unwrap();
    assert!(result.is_empty(), "Reference in document with front matter should be detected as used");
}
...
fn test_multiline_definition() {
    ...
    let result = rule.check(content).unwrap();
    assert!(result.is_empty(), "Multiline reference definition should be properly detected");
}
...
fn test_code_span_in_reference() {
    ...
    let result = rule.check(content).unwrap();
    assert!(result.is_empty(), "Reference with code span should be properly detected");
}
...
fn test_performance_fix() {
    ...
    for i in 1..101 {
        if i % 2 == 1 {
            assert!(!fixed.contains(&format!("[ref{}]:", i)), 
                    "Unused reference [ref{}] should be removed", i);
        } else {
            assert!(fixed.contains(&format!("[ref{}]:", i)), 
                    "Used reference [ref{}] should be kept", i);
        }
    }
    
    // Verify performance is reasonable
    println!("MD053 fix performance test completed in {:?}", duration);
    assert!(duration.as_millis() < 1000, "Fix operation should complete in under 1000ms");
}
