...
**Custom Heading**";
    ...
    assert!(result.len() >= 1, "Should detect at least one emphasis");
...
More __bold __ with spaces";
    ...
    assert!(result036.len() >= 1, "MD036 should detect headings");
    assert!(result037.len() >= 2, "MD037 should detect spaces");
    assert!(result049.len() >= 1, "MD049 should detect wrong style");
    assert!(result050.len() >= 1, "MD050 should detect wrong style");
}
...
___both___";
    ...
    assert!(!result049.is_empty() || !result050.is_empty(), "Should detect some style issues");
}