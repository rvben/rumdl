...
fn test_md009_last_line_handling() {
    ...
    let ctx = LintContext::new(content_with_newline, rumdl_lib::config::MarkdownFlavor::Standard);
    let result = rule.check(&ctx).unwrap();
    ...
    let ctx = LintContext::new(content_without_newline, rumdl_lib::config::MarkdownFlavor::Standard);
    let result = rule.check(&ctx).unwrap();
