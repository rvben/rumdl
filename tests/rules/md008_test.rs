...
fn test_lists_with_blank_lines() {
    ...
    let content = "* Item 1\n\n* Item 2\n\n- Item 3";
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 1, "List with blank lines should still check consistency");
...
fn test_lists_in_blockquotes() {
    ...
    let content = "> * Item 1\n> * Item 2\n> - Item 3";
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 0, "Current implementation doesn't detect inconsistent list markers in blockquotes");
...
fn test_lists_with_html_comments() {
    ...
    let content = "* Item 1\n<!-- * This is a comment -->\n* Item 2\n- Item 3";
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 1, "List with HTML comments should still check consistency");
...
fn test_complex_nested_lists() {
    ...
    let content = "* Top level 1\n  - Nested 1.1\n    + Deep nested 1.1.1\n* Top level 2\n  * Nested 2.1";
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 2, "Nested list should check consistency at all levels");
    
    // Fix should handle all levels
    let fixed = rule.fix(content).unwrap();
    let expected = "* Top level 1\n  * Nested 1.1\n    * Deep nested 1.1.1\n* Top level 2\n  * Nested 2.1";
    assert_eq!(fixed, expected);
}
...
fn test_with_front_matter() {
    ...
    let content = "---\ntitle: Test Document\n---\n\n* Item 1\n* Item 2\n- Item 3";
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 1, "Front matter should be ignored when checking lists");
...
fn test_mixed_ordered_unordered_lists() {
    ...
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 1, "Should check only unordered list items consistency");
...
fn test_indentation_variations() {
    ...
    let result = rule.check(content).unwrap();
    assert_eq!(result.len(), 1, "Should identify inconsistent markers despite indentation variations");
...
fn test_list_markers_at_edge_of_line() {
    ...
    assert_eq!(result.len(), expected_warnings, "Edge case with no space after marker should be handled gracefully");
}
