...
fn test_front_matter_title_pattern() {
    ...
    let rule = MD041FirstLineHeading::with_pattern(1, true, Some("^(title|header):".to_string()), false, false);
    ...
    let rule = MD041FirstLineHeading::with_pattern(1, true, Some("^Title:".to_string()), false, false);
    let content = "---\nTitle: My Document\n---\nContent";
    let ctx = LintContext::new(content, rumdl_lib::config::MarkdownFlavor::Standard, None);
    let result = rule.check(&ctx).unwrap();
