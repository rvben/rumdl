...
use rumdl::rule::Rule;
use rumdl::rules::{MD004UnorderedListStyle, MD005ListIndent, MD007ULIndent, MD029OrderedListPrefix};
...
* Third item back to asterisk";
    ...
    let ctx_after_md004 = LintContext::new(&md004_fixed);
...
4. Fourth item";
    ...
    let ctx_after_md029 = LintContext::new(&md029_fixed);
...
* Back to proper unordered";
    ...
    let ctx3 = LintContext::new(&step3);

    let step4 = md029.fix(&ctx3).unwrap();
    let ctx_final = LintContext::new(&step4);
...
+ Second item with wrong marker
  ...
  * Nested item
    ...
    let md004_fixed = md004.fix(&ctx).unwrap();
    let ctx_fixed = LintContext::new(&md004_fixed);
