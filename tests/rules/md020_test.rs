...
fn test_invalid_closed_atx_headings() {
    ...
    assert_eq!(result[0].message, "Missing space inside hashes on closed ATX style heading with 1 hashes");
}
...
fn test_heading_with_multiple_hashes() {
    ...
    assert_eq!(result[0].message, "Missing space inside hashes on closed ATX style heading with 6 hashes");
