...
fn test_emphasis_wrapping_multiple_sentences() {
    // Emphasis spanning multiple sentences
    let rule = create_sentence_per_line_rule();
    let content = "**First sentence. Second sentence.**\n";
    let ctx = LintContext::new(content, rumdl_lib::config::MarkdownFlavor::Standard, None);
    let result = rule.check(&ctx).unwrap();
    ...
    // Each sentence gets its own line. The span is not reopened per line: a line
    // break inside emphasis is whitespace, and duplicating the markers would
    // delete the space the closing marker replaces.
