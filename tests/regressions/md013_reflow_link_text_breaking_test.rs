...
#[test]
fn test_bare_list_item_link_breaks_within_text() {
    let dir = TempDir::new().unwrap();
    let content = "- [A long link description with multiple words](https://e.com/x)\n";
    let settings = ["reflow = true", "line-length = 40", "reflow-break-link-text = true"];
    let formatted = fmt(dir.path(), content, &settings);

    assert_ne!(formatted, content, "a bare link in a list item should wrap");
    assert!(formatted.contains("  words](https://e.com/x)"));
}

#[test]
fn test_bare_list_item_image_breaks_within_alt_text() {
    let dir = TempDir::new().unwrap();
    let content = "- ![A long image description with multiple words](https://e.com/x)\n";
    let settings = ["reflow = true", "line-length = 40", "reflow-break-link-text = true"];
    let formatted = fmt(dir.path(), content, &settings);

    assert_ne!(formatted, content, "a bare image in a list item should wrap");
    assert!(formatted.contains("  multiple words](https://e.com/x)"));
}
