...
//! A code span starts on a backtick and a lowercase word starts on a lowercase
//! letter, so both held the sentence open, and the sentence after it was
//! absorbed. The joined line then exceeded `line-length`, which the same rule
//! reported as a line-length violation carrying no fix — `fmt` produced output
//! `check` rejects, and a second `fmt` could not repair it.
//!
...
/// Every shape #811 lists, plus the two the boundary rule has to read the same
/// way: a span that closes inside a quotation, and a trailing code span with no
/// text element after it to re-split the line.
fn cases() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            "bold_then_lowercase",
            "**An enum column may not be an atom a keyset scan resumes from.**\nnpm would follow the declaration index while the keyset predicate followed the value.\nThe two disagree, and the page skips rows.\n",
        ),
        (
            "bold_then_uppercase",
            "**An enum column may not be an atom a keyset scan resumes from.**\nOrder by would follow the declaration index while the keyset predicate followed the value.\nThe two disagree, and the page skips rows.\n",
