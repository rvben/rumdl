...
"#;
        ...
        let warnings = rumdl_lib::lint(content, &md054_rules, false, MarkdownFlavor::Standard, None, None).unwrap();
    ...
    fn test_md054_off_by_one_bug() {
        ...
        for (i, content) in test_cases.iter().enumerate() {
            // The test passes if it doesn't panic due to indexing errors
            let result = rumdl_lib::lint(content, &md054_rules, false, MarkdownFlavor::Standard, None, None);
            assert!(
                result.is_ok(),
