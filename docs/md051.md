...
A cross-file fragment is checked in the file the link leads to, found the way [MD057](md057.md) finds it: the path is percent-decoded, a
link without an extension tries the Markdown extensions, and MD057's `search-paths`, `absolute-links` and `roots` settings apply. A
destination with a path always leads to another file, so its fragment is never checked against the linking document. A link whose file
does not exist is reported by MD057. MD051 then checks its fragment only in the language server, when the file is a document the
editor has open but not yet saved.
...
| `true` | A fragment in a frontmatter value must name an existing anchor |
...
Enable it for projects whose frontmatter links really are relative to the
document, and list the keys that are not under `ignore-frontmatter-fields`.

A fragment-only value is resolved against this document's own headings. A value
carrying a path resolves against the target file, exactly like a cross-file body
link:

```yaml
---
anchor: '#setup'          # must match a heading in this document
guide: ./install.md#setup # must match a heading in install.md
canonical: https://example.com/a.md#setup  # external, not checked
title: Node.js            # prose, not read as a link
---
```

`ignore-case` and `ignored-pattern` apply to frontmatter fragments exactly as
they do to body links.
...
An HTML element in the document is a link target through its `id` attribute, whatever the element. An `<a>` element is also a target through its legacy `name` attribute, the only element for which browsers honour `name` when navigating to a fragment:

```markdown
<a name="legacy" id="modern"></a>
<div id="section-id"></div>

[both work](#legacy) and [so does this](#modern) and [this](#section-id)
```

Markup inside inline code or an HTML comment is not rendered, markup inside an image description is alt text, and a backslash-escaped `\<` is literal text, so none of these defines a target. Inside an HTML block no escapes are processed, so a backslash before a tag there is plain text and the element is still a target. Tag names there follow the browser, which accepts any character but whitespace, `/` and `>` after the first letter, where CommonMark's inline grammar accepts only letters, digits and hyphens.
