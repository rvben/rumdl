...
Every other rule reads a line with no space after its `#`s the way CommonMark renders it: as paragraph text, not a heading. So a tag is never reported or rewritten by the heading rules (MD022, MD025 and the rest), whatever its case, and a line such as `#Title` only becomes a heading for them once this rule's fix adds the space.
