...
whole-words = false              # Preserve names inside compound identifiers (default: false)
...
- Set `whole-words = true` to match a configured name only when it occupies a
  whole prose token. With this option, runs of hyphens and underscores between
  Unicode letters or numbers join identifier components, so `abc` is checked
  in `(abc)` and `[abc]` but not in `123-abc-def`, `foo_abc_bar`,
  `foo__abc__bar`, or Unicode compounds such as `café_abc_menu`. Underscores
  parsed as Markdown emphasis delimiters do not join tokens: `_abc_`,
  `__abc__`, and the emphasized `abc` in `foo-__abc__-bar` are still checked.
  Emphasizing an entire identifier, such as `_foo_abc_bar_` or `_abc_def_`,
  still protects its embedded components. The default is `false` for
  markdownlint compatibility, so compound components continue to be checked
  unless this option is enabled. This setting does not add a general path or
  URL exemption; existing link and URL exclusions still apply.
...
- Won't check inside a template shortcode tag (`{{% nodejs %}}`,
  `{{< figure alt="nodejs" >}}`). A shortcode is resolved by name against a
  template, so the text in the tag names what to invoke rather than mentioning
  a product, and rewriting it points the invocation at a template that does not
  exist. Only the tag itself is exempt: prose on the same line is still checked,
  and so is the body between a paired `{{% note %}}` and `{{% /note %}}`, which
  the site generator renders as ordinary Markdown
