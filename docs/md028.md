...
Flags blank lines that appear between consecutive blockquotes when there is no other content separating them, but only in flavors where that boundary is not defined as a separate quote.
...
CommonMark and GFM define a blank line without `>` markers as two separate blockquotes. The `standard` flavor (including the `gfm`, `github`, and `commonmark` aliases) and the `obsidian` flavor therefore do not report this pattern. Other flavors retain the portability check.
...
### ❌ Incorrect - Ambiguous consecutive blockquotes in other flavors
...
The blank line between these blockquotes is ambiguous in flavors that do not define this boundary:
...
**Option 3:** Use the `standard` or `obsidian` flavor when the target renderer follows CommonMark/GFM and you intentionally want separate blockquotes without intervening content.
