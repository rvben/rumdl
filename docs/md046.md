...
- Convert code blocks when the target representation is lossless
- Keep language-tagged fences intact when `style = "indented"`, because
  indented code blocks cannot carry an info string
- Reject a fenced-to-indented conversion if reparsing would merge a block or
  turn its contents into prose
...
- When using "consistent", the most prevalent style is normally used (in case
  of a tie, fenced style is preferred). If converting to indented blocks would
  discard an info string or change the parsed code-block structure, fenced
