...
The automatic fix collapses flagged violations to a single space. Only spaces
that exceed `max-consecutive-spaces` are fixed - allowed spaces are preserved.
...
| ------------------------- | ------- | ------- | ---------------------------------------- |
| `max-consecutive-spaces`  | integer | 1       | Maximum allowed consecutive spaces       |

### max-consecutive-spaces

The `max-consecutive-spaces` option controls how many consecutive spaces are
allowed before triggering a warning. The default value of `1` flags any
occurrence of 2 or more consecutive spaces.

Setting this to `2` allows the traditional "two spaces after a period"
typographical convention while still flagging 3+ consecutive spaces:
...
```markdown
This is fine.  Two spaces between sentences.    <- OK
This is not.   Three spaces here.               <- Flagged
```
