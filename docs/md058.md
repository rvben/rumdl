...
directly above or below a table satisfies this rule and `rumdl fmt` leaves the
document alone:
...
paragraph, so a table written under it is still missing its blank line. A
comment spanning several lines counts at both ends, and the convention holds
inside a blockquote. [MD022](md022.md), [MD031](md031.md) and [MD032](md032.md)
