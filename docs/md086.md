...
`<!--` below one is inside it and is not reported separately. The reverse is
reported: a `%%` below an unclosed `<!--` is still a closer the author has to
add, so it is listed alongside the opener above it rather than dropped.
