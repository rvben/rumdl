...
Obsidian hides everything from an unclosed `%%` to the end of the note, so a
`<!--` below one is inside it and is not reported separately. An unclosed `<!--`
does not work the same way: it only swallows what follows when it starts a line,
and mid-paragraph it renders as literal text. A `%%` below one is therefore
reported on its own.
