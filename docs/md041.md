...
allow-preamble = false  # if enabled, allows prose preamble before the first heading
```
...
- If `allow_preamble` is enabled, the prose before the first heading will be ignored.
  That heading must be at the configured `level`.
  If no heading is found, the rule reports a violation, as is `allow_preamble` was disabled.
