...
- **Unique Features**: <!-- RULE_COUNT_ADDITIONAL -->35<!-- /RULE_COUNT_ADDITIONAL --> additional rules (MD057, MD061-<!-- RULE_MAX -->MD094<!-- /RULE_MAX -->), built-in LSP server, VS Code extension, and built-in Markdown flavors
...
rumdl implements **<!-- RULE_COUNT -->88<!-- /RULE_COUNT --> rules total**: all 53 markdownlint rules plus <!-- RULE_COUNT_ADDITIONAL -->35<!-- /RULE_COUNT_ADDITIONAL --> unique rules.
...
rumdl implements <!-- RULE_COUNT_ADDITIONAL -->35<!-- /RULE_COUNT_ADDITIONAL --> additional rules not found in markdownlint:
...
| MD090  | No HR before heading           | Flags a horizontal rule directly above a heading (opt-in)  |
| MD091  | No markdown in HTML            | Flags markdown inside an HTML block, which renders as text (opt-in) |
| MD092  | Merge conflict markers         | Detects unresolved conflicts and prevents formatting               |
| MD093  | No formatting in headings      | Flags inline formatting inside a heading (opt-in)          |
| MD094  | Invalid encoding               | Reports bytes that are not valid UTF-8, and binary files   |

**Opt-in rules:** MD060, MD063, MD070, MD072, MD073, MD074, MD080, MD082,
MD083, MD084, MD085, MD087, MD088, MD089, MD090, MD091, and MD093 are disabled by default. Enable them
...
| ------------------------ | ------------------ | --------------------------- |
| **Core Functionality**   |                    |                             |
| Rule count               | 53 implemented     | <!-- RULE_COUNT -->88<!-- /RULE_COUNT --> (53 compatible + <!-- RULE_COUNT_ADDITIONAL -->35<!-- /RULE_COUNT_ADDITIONAL --> new) |
| Auto-fix                 | ✅                 | ✅                          |
