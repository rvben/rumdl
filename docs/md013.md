...
- **Reflow.** `ignore-link-urls` forgives a *violation*; it does not change how a
  line is *measured* when reflow rewraps a paragraph. Reflow always measures the
  markdown as written, including link destinations, so a paragraph whose only
  over-length cause is a link URL is still rewrapped and the link still lands on
  its own line. This matches prettier (`--prose-wrap always`) and mdformat
  (`--wrap`), which both wrap such a paragraph the same way. Set `reflow = false`
  to keep those lines as authored. rumdl prints a config warning when
  `ignore-link-urls` is set alongside `reflow`, so the boundary is not silent.
