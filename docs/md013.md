...
- `require-sentence-capital`: Whether a sentence boundary requires the next sentence to start with an uppercase letter (default: `true`)
  ...
  - A code span opens a sentence whatever the setting: it starts on a backtick rather than on a letter, and the case of what it holds belongs to the code, so `Reads the file. \`config.toml\` holds the rest.` is two sentences. Not after a digit, so an enumeration such as `Steps: 1. \`init\` the repo, 2. \`build\` it.` stays one
  - A sentence that both opens and closes inside one emphasis, strong or strikethrough span is delimited by that span's own markers, so `**An enum column is not an atom.** npm reads it as one` is two sentences. A span that closes mid-sentence is not: `Click **Save.** then restart.` and `Select the **New File...** button to create a file.` stay one sentence each, and so does a `**Note:**` lead-in followed by a later span. A literal `*`, `_` or `~` inside the span suppresses the split rather than risking it
