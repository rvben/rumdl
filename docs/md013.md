...
- **Reflow.** `ignore-link-urls` decides whether a line is *reported*; it does not
  change how reflow *measures* one. Reflow always measures the markdown as
  written, including link destinations, so a paragraph whose only over-length
  cause is a link URL is still rewrapped and the link still lands on its own
  line. prettier (`--prose-wrap always`) and mdformat (`--wrap`) wrap such a
  paragraph the same way. `stern` and `code-spans` leave reflow alone in the
  same way. `strict` is the exception: it removes the standalone-link and
  HTML-line exemptions everywhere, including inside reflow, so it can make
  reflow report a paragraph that is otherwise left alone. Set `reflow = false`
  to keep those lines as authored.
...
An ordered marker counts only when it could genuinely interrupt the paragraph,
which means numbered 1 with content after it. `7. item` and a bare `123456.`
are prose to the parser, so a line may wrap before them.
