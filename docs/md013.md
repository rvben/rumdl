...
bracket-display-math = false  # Protect standalone \[ ... \] TeX display math (default: false)
...
reflow-break-link-text = false  # Allow reflow to wrap long lines within link/image text (default: false)
...
- `bracket-display-math`: Recognize standalone TeX `\[ ... \]` blocks as display math (default: `false`). Enable this only for a renderer that treats those delimiters as math. A matched multi-line block keeps its authored line breaks during reflow, including inside lists and blockquotes; a complete `\[ ... \]` line stays separate from adjacent prose. An unmatched opener is ordinary Markdown. With this option on, `math-blocks = false` also exempts recognized bracket display math from the width check. With it off, escaped brackets keep their normal CommonMark meaning.
...
- `require-sentence-capital`: Whether a sentence boundary requires the next sentence to start with an uppercase letter (default: `true`)
  ...
  - A code span opens a sentence whatever the setting: it starts on a delimiter rather than on a letter, and the case of what it holds belongs to the code, so `Reads the file. \`config.toml\` holds the rest.` is two sentences. The code span has to be one the parse recognises, since an unmatched backtick is ordinary text and the sentence carries on through it. Not after one mark of an elision, so `Choose File... \`Open Folder\` appears in the menu.` stays one, and not after a bare digit, so the enumeration `Steps: 1. \`init\` the repo, 2. \`build\` it.` and the version `Pinned at 0.2.43. \`rumdl\` reads it.` stay one too. A digit inside a span is a label's rather than an enumerator's, so `**A2.** \`UserProvider\` re-mints the token.` is two
...
- `reflow-break-link-text`: Whether reflow may wrap a long line within the text of a link or image (default: `false`). Off, every link and image is one atomic token: an overlong link moves whole to its own line, which the check exempts as a standalone link. On, the bracketed text of `[text](url)` wraps at whitespace under the same rules `atomic-spans` applies to emphasis spans: only when the whole construct is longer than `line-length` (or freely, when `atomic-spans` is `false`). The inline, full (`[text][ref]`), collapsed (`[text][]`) and shortcut (`[text]`) forms and the corresponding image forms all wrap alike; reference labels normalize internal whitespace, so a wrapped label still resolves. The `](...)` tail is never split, and a link stays whole whenever splitting would produce a line the check reports but reflow cannot fix: when the destination alone exceeds the limit while the text fits (splitting could not shorten anything the URL exemption does not already forgive), or when the tail line's overflow would not be confined to its final token (for example a title pushed past the limit)
...
- **Reflow, by default.** `ignore-link-urls` decides whether a line is
  *reported*; on its own it does not change how reflow *measures* one. Reflow
  ...
  so it can make reflow report a paragraph that is otherwise left alone. The
  sentence-driven modes differ in the other direction: `sentence-per-line` and
  `semantic-line-breaks` shape a paragraph by its sentences, so a link line that
  continues the prose above it is joined into that sentence instead of standing
  on its own. Set
...
- Reflows definition list definitions, indenting wrapped lines to the definition's text (see [Definition list reflow](#definition-list-reflow))
...
### Definition list reflow

A definition (the text after a `: ` marker) reflows like a paragraph:

- The first line keeps the marker and the spacing after it as written
- Wrapped lines are indented to the column where the definition's text starts,
  and to at least four columns. Python-Markdown (MkDocs) reads a line indented
  less than four as part of the definition only when no other definition
  follows it, so a narrower indent could turn the text into a new term
- A later paragraph of a definition keeps the indentation it has
- Terms are never reflowed
- A list or blockquote inside a definition reflows the way it does anywhere else
- A definition whose text a hard break splits, with a line after the break
  indented less than the text, is left as written: indenting one part and
  not the other would end the definition at that line in Python-Markdown

Example (`line-length = 40`):
...
- Ends a sentence that closes inside an emphasis, strong or strikethrough span.
  The closing markers are read as a run, so any length and any nesting ends the
  sentence they close: `~Struck.~`, `**Bold.**`, `***Emphasized.***` and
  `**_Bold ital._**` all break before the sentence that follows. A closing quote
  may sit ahead of the run, as in `He said "Done."** Next.`
- Treats a line holding nothing but a link or image as content rather than as a
  paragraph boundary. Where such a line continues the sentence above it, the
  sentence is joined back onto one line. The width-driven modes (`default`,
  `normalize`) instead leave that line where the author put it, since joining it
  would build a line the width check then refuses to wrap. A link line that is
  its own paragraph is left alone in every mode
