...
- **Reflow, by default.** `ignore-link-urls` decides whether a line is
  *reported*; on its own it does not change how reflow *measures* one. Reflow
  ...
  `reflow-length-exemptions = true` to have reflow honour this exemption, or
...
discounting code spans does, never when only the two savings together would. A
combination the option cannot produce is a line the check reports but reflow
declines to wrap: reflow is at least as willing to wrap as the check is to
complain.
