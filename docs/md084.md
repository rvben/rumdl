...
An attached selector is never reported or removed, but it still counts as an invisible
character when looking for consecutive ones, so nothing can hide behind an emoji. A
zero-width space in `⚠️​x` is reported, and the auto-fix removes only the zero-width
space.
