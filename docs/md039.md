...
Reference-style links and images (`[ text ][id]`, `![ alt ][id]`) are not checked,
matching markdownlint.

Wikilinks and wiki embeds (`[[page| Display text ]]`, `![[image.png| 300 ]]`) are left
alone. Trimming them would mean rewriting them as `[text](destination)`, and they carry
