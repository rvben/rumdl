...
emitted only when none of the configured roots contain the target.
...
or paths relative to the current working directory; trailing slashes are normalized.

When `roots` is empty, the rule emits a "not validated" warning for every absolute link,
consistent with the `relative_to_docs` fallback when no `mkdocs.yml` is found.
...
file's directory. Paths are resolved relative to the project root (where `.rumdl.toml` or
