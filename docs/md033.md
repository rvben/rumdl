...
- Tags inside a template shortcode tag: `{{% note <b>x</b> %}}`. The text there
  is a string the template receives, not HTML the document emits. Only the tag
  is exempt; HTML in the body between a paired opening and closing shortcode is
  still reported
