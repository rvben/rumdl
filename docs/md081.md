...
**Disabled by default.** Both thresholds default to `0` (off), so the rule emits
nothing until you set a limit. There is no markdownlint equivalent; this is a
rumdl-specific readability rule.
...
| `max-per-paragraph` | integer | `0` | Maximum emphasis spans allowed per paragraph. `0` disables this check. |
| `max-consecutive` | integer | `0` | Maximum length of a run of adjacent emphasis spans. `0` disables this check. |
